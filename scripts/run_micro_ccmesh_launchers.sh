...
./c2.sh run-gateway gateway.$oarid micro-ccmesh
./c2.sh run-snitch gateway.$oarid

sleep 5

./c2.sh run-engines workers.$oarid micro-ccmesh gateway.$oarid

for wrkr in ${workers[@]}; do
	./c2.sh run-launcher $wrkr micro-ccmesh 1 gateway.$oarid
	./c2.sh run-launcher $wrkr micro-ccmesh 2 gateway.$oarid
done
