...
// SCALE LAMBDA
pub async fn mesh_service0(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    let mut workloads = VecDeque::with_capacity(NLAMBDA);
    for _ in 0..NLAMBDA {
        workloads.push_back(get_21());
    }
...
pub async fn mesh_service2(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    c.local.merge_into(&h1.local);
    c.deps.merge_into(&h2.deps);
    c.local.merge_into(&h2.local);
    c.deps.merge_into(&h3.deps);
    c.local.merge_into(&h3.local);
    c.deps.merge_into(&h4.deps);
    c.local.merge_into(&h4.local);
...
pub async fn mesh_service3(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    let workload: Workload = vec![];
    let mut c = GoClient::new();
    c.workload = workload;
