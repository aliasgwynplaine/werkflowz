...
$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    ...
    --v=1 2>$BASE_DIR/outputs/writer.log &

$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    --func_id=2 --fprocess_mode=go \
    --fprocess_output_dir=$BASE_DIR/outputs \
    --fprocess=$BASE_DIR/readerwriter \
    --v=1 2>$BASE_DIR/outputs/readerwriter.log &

$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    --func_id=3 --fprocess_mode=go \
    --fprocess_output_dir=$BASE_DIR/outputs \
    --fprocess=$BASE_DIR/finalreader \
    --v=1 2>$BASE_DIR/outputs/finalreader.log &
