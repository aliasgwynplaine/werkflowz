...
func (h *readerwriterHandler) Call(ctx context.Context, input []byte) ([]byte, error) {
	...
	client := ccmesg.NewMeshGoClient("readerwriter")
	RunId := string(input)
	//client.InitTxn()
	value := client.Read("1")
	fmt.Println(RunId, "1 = ", value)
	new_value := strconv.Itoa(rand.Intn(9999) + 1)
	client.Write("2", new_value)
	fmt.Println(RunId, "2 <- ", new_value)
	client.CommitTxn()
	fmt.Println("-----------------")
	...
	*/
	...
	invokeurl := "http://" + ccmesg.NIGHTCORE_GW_ADDR + ":8080/function/finalreader"
